...
function initRuntimeLog() { ensureDirectory(); logRuntime('startup', 'version=' + app.getVersion() + ' platform=' + process.platform); process.on('uncaughtException', (error) => logRuntime('uncaughtException', error?.stack || error)); process.on('unhandledRejection', (reason) => logRuntime('unhandledRejection', reason?.stack || reason)); process.once('SIGTERM', () => { logRuntime('signal', 'SIGTERM'); process.exit(143); }); process.once('SIGINT', () => { logRuntime('signal', 'SIGINT'); process.exit(130); }); app.on('render-process-gone', (_event, details) => logRuntime('render-process-gone', JSON.stringify(details))); app.on('before-quit', () => logRuntime('before-quit')); }
